...
1) Создать в сцене пустой объект GameplayManager и сделать его синглтоном:
```cs
using UnityEngine;
...
```
2) Добавить в него функции, которые изменяют глобальный state игры:
```cs
...
```
3) Вызывать эти функции соответственно из щита при подбирании яица и из яица при столкновении с землёй.
...
1) Добавить в GameplayManager код создания щита произвольного размера:
```cs
...
```
2) Добавить код уменьшения жизней и рестарта сцены:
```cs
...
3) Импортировать и добавить на сцену ассет skybox.
4) Импортировать и добавить ассеты окружения (low poly forest).
5) Добавить текстовое поле с "Defeat", которое появляется после поражения на несколько секунд.

!["screenshot"](Screenshots/3.webp)

#### Работа 4: «Структурирование исходных файлов в папке»
1) Все использованные сторонние ассеты с зависимостями (пкм -> Select Dependencies) перенести в соответствующие папки проекта.
2) Удалить папки с оставшимися импортированными ассетами.
3) Отсортировать ассеты проекта по принадлежности к объекту. Отсортировать ассеты по типу можно при вводе в поиск t:prefab или t:material или т.п.
